...
local ns = api.nvim_create_namespace('tausi')

local function gen_shortcut(config)
  local shortcut = config.shortcut
      or {
        { desc = '[  Github]', group = 'TausiShortCut' },
        { desc = '[  glepnir]', group = 'TausiShortCut' },
        { desc = '[  0.2.3]', group = 'TausiShortCut' },
      }
  ...
  for _, item in pairs(shortcut) do
    ...
    if item.icon then
      api.nvim_buf_add_highlight(
        config.bufnr,
        0,
        item.icon_hl or 'TausiShortCutIcon',
        first_line,
        start,
        start + #item.icon
      )
...
local function project_list(config, callback)
  config.project = vim.tbl_extend('force', {
    ...
    enable = true,
    icon = '󰏓 ',
    icon_hl = 'TausiRecentProjectIcon',
...
local function gen_center(plist, config)
  ...
  api.nvim_buf_add_highlight(config.bufnr, 0, 'TausiProjectTitle', first_line + 1, 0, -1)
  api.nvim_buf_add_highlight(
    config.bufnr,
    0,
    'TausiProjectTitleIcon',
    first_line + 1,
    0,
  ...
  api.nvim_buf_add_highlight(config.bufnr, 0, 'TausiMruTitle', first_line + plist_len, 0, -1)
  api.nvim_buf_add_highlight(
    config.bufnr,
    0,
    'TausiMruIcon',
...
local function gen_footer(config)
  ...
  for i, _ in pairs(footer) do
    api.nvim_buf_add_highlight(config.bufnr, 0, 'TausiFooter', first_line + i - 1, 0, -1)
  end
end

local function project_delete()
  api.nvim_create_user_command('DbProjectDelete', function(args)
    local path = utils.path_join(vim.fn.stdpath('cache'), 'tausi', 'cache')
    utils.async_read(
      path,
      vim.schedule_wrap(function(data)
        local dump = assert(loadstring(data))
...
local function theme_instance(config)
  project_list(config, function(plist)
    ...
    require('tausi.theme.header').generate_header(config)
    gen_shortcut(config)
    load_packages(config)
    gen_center(plist, config)
    gen_footer(config)
    map_key(config, config.confirm_key or '<CR>')
    require('tausi.events').register_lsp_root(config.path)
