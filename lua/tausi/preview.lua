...
function view:open_window(opt)
  ...
  api.nvim_buf_set_option(self.bufnr, 'filetype', 'tausipreview')
  self.winid = api.nvim_open_win(self.bufnr, false, opts)
  if vim.fn.has('nvim-0.8') == 1 then
    local normal = api.nvim_get_hl_by_name('Normal', true)
    pcall(api.nvim_set_hl, 0, 'TausiPreview', normal)
  else
    api.nvim_set_hl(0, 'TausiPreview', { bg = 'none' })
  end
  api.nvim_win_set_option(self.winid, 'winhl', 'Normal:TausiPreview')
...
function view:preview_events()
  local group =
      api.nvim_create_augroup('TausiClosePreview' .. self.preview_bufnr, { clear = true })
  ...
  api.nvim_create_autocmd('BufEnter', {
    ...
    desc = 'Tausi close or regenerate preview window',
  })
  ...
  local function winresized()
    api.nvim_create_autocmd('WinResized', {
      ...
      desc = ' Tausi preview window resized for nvim 0.9',
    })
  end
  ...
  if vim.fn.has('nvim-0.9') == 1 then
    ...
    api.nvim_create_autocmd('BufEnter', {
      ...
      desc = 'tausi preview window resize for neovim 0.8+ version',
    })
  end
end
